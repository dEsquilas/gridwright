 ...
 *
 * Static HTML, no server, no build step. The images stay where the pipeline
 * wrote them and the page points at them: inlining them made a ten-section view
 * a 74MB file nothing would open.
 */
...
<p class="muted" style="margin-top:36px;font-size:12px">
  ...
  Images are read from <code>.gridwright/</code> beside this page — keep them together.
</p>
...
function designImage(root: string, name: string, run: RunState | null): string | null {
  return imageRef(root, join(paths.baseline(root, name), 'figma.png'))
    // Flat names, from before each thing got a folder.
    ?? imageRef(root, join(paths.baselines(root), `${name}.figma.png`))
    ?? imageRef(root, join(paths.baselines(root), `${name}.design.png`))
    ?? (run ? imageRef(root, paths.reference(root, run.id)) : null)
}
...
function renderImage(root: string, name: string, viewport: string, run: RunState | null): string | null {
  const own = imageRef(root, join(paths.baseline(root, name), `${viewport}.png`))
  if (own) return own
  ...
  if (!legacy) {
    const frozen = imageRef(root, join(paths.baselines(root), `${name}.${viewport}.png`))
...
function shot(root: string, runId: string, file: string): string | null {
  return imageRef(root, join(paths.runVerify(root, runId), file))
    ?? imageRef(root, join(paths.verify(root), file))
}
...
/**
 * Where an image is, relative to the dashboard — not the image itself.
 *
 * Images used to be inlined as base64, so the page was one file that opened
 * anywhere. A view with ten sections took that to 74MB — 70 images, 15MB of
 * them the Figma export of the whole page at 2x — past what a viewer will open
 * at all, and every one of them loaded up front to look at one. They are
 * already on disk under `.gridwright/`, the baselines committed, so the page
 * points at them and the browser loads what the open entry shows.
 *
 * `<img>` from `file://` is allowed; it was `fetch` that is not, which is what
 * the inlining was guarding against in the first place.
 */
function imageRef(root: string, path: string): string | null {
  if (!existsSync(path)) return null
  const rel = relative(paths.dashboard(root), path).split(sep).map(encodeURIComponent).join('/')
  return rel
}
